...
const Modal = ({ i, onClose, onAlert }: ModalProps) => {
    ...
    return (
        <>
            {i === 1 ? (<>
                <div
                    className='fixed w-full h-full top-0 right-0 backdrop-blur-md bg-white/40'>
                    <div
                        className='bg-white absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-10 rounded shadow-xl'
                    >
                        <div className='relative'>
                            ...
                            <div className='grid grid-cols-8 gap-8'>
                                <input
                                    type="text"
                                    className='p-3 text-gray-600 col-span-4 rounded-md bg-slate-50'
                                    placeholder='Name'
                                ...
                                <select
                                    ...
                                    name=""
                                    id=""
                                    className='col-span-4 p-3 text-gray-400 rounded-md selection:text-gray-600 bg-slate-50'>
                                ...
                                <input
                                    ...
                                    className='p-3 text-gray-600 col-span-5 rounded-md bg-slate-50'
                                    placeholder='Description'
                                    required
                                />
                                <input type="number"
                                    className='col-span-3 rounded-md p-3 text-gray-600 bg-slate-50'
                                    placeholder='Quantity'
                                    onChange={(e) => setQty(e.target.value)}
                                    required
                                />
                                <input type="number"
                                    className='col-span-4 rounded-md p-3 text-gray-600 bg-slate-50'
                                    placeholder='Price'
                                    required
                                    onChange={(e) => setPrice(e.target.value)}
                                />
                                <input type="file"
                                    className='col-span-4 rounded-md p-3 text-gray-600 border-none bg-slate-50 '
